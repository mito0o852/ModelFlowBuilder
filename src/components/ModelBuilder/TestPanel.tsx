...
import { Play, AlertCircle, CheckCircle2, ChevronDown, ChevronUp } from "lucide-react";
...
const TestPanel = ({
  ...
  return (
    <Card 
      className={`fixed bottom-0 bg-white border-t z-50 transition-all duration-300 ease-in-out shadow-lg ${
        sidebarOpen ? 'right-[300px]' : 'right-0'
      } left-[280px]`}
    >
      <div 
        className="absolute -top-8 right-4 bg-white rounded-t-lg border border-b-0 px-3 py-1 cursor-pointer hover:bg-gray-50"
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[400px]' : 'max-h-[60px]'}`}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="input" className="px-3 mt-3">
              <div className="flex flex-wrap items-center gap-3">
                <div className="text-xs text-muted-foreground whitespace-nowrap">Sample Input:</div>
            ...
            <TabsContent value="validation" className="px-3 mt-3 mb-4">
              {isModelValid ? (
                ...
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Please check for disconnected nodes or incompatible dimensions.
                  </AlertDescription>
                </Alert>
              )}
            </TabsContent>
